...
pub fn main() !void {
...

    {
        ...
        const handle1 = try std.Thread.spawn(.{}, thread_pi, .{ &pi_plus, 5, count, Operation.ADD });
        defer handle1.join();

        // Second thread to calculate the minus numbers.
        const handle2 = try std.Thread.spawn(.{}, thread_pi, .{ &pi_minus, 7, count, Operation.SUBTRACT });
        defer handle2.join();
    }
...
fn thread_pi(pi: *f64, begin: u64, end: u64, sign: Operation) !void {
    var local_sum: f64 = 0;
    var n: u64 = begin;
    const multiplier: f64 = if (sign == Operation.ADD) 1.0 else -1.0;
    while (n < end) : (n += 4) {
